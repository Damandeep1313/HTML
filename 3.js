...
app.post('/generate-landing-page', (req, res) => {
  try {
    const {
      ...
      doctorDetails,     // e.g. { name: "Dr. daman Singh", specialization: "Neuro", achievements: "..."}
      pageLinks = [],    // e.g. ["about-us", "services", "contact"]
      
      // Single or multiple images for hero (used for hero & about)
      images = [],
      
      // NEW: Array for testimonial images
      testimonialImages = []
    } = req.body;
    ...
    // We’ll grab the first image from images[] or fallback:
    const mainImage = images.length > 0
      ? images[0]
      : 'https://via.placeholder.com/1200x600?text=No+Hero+Image';

    // We’ll grab up to 3 images for testimonials, fallback if not provided
